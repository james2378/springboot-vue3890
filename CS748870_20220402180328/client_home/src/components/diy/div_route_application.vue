<template>
	<div class="diy_details_box diy_div_route_application">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "route_application_id",
					source_id: this.obj["route_application_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/route_application/set?route_application_id=' + _this.obj["route_application_id"], {
								praise_len
							}, (res) => {
								if(res.result){
