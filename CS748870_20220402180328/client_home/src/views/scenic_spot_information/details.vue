<template>
	<div class="diy_details page_scenic_spot_information" id="scenic_spot_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_scenic_spot_information'>
							<div_scenic_spot_information :obj="obj"></div_scenic_spot_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_scenic_spot_information from "@/components/diy/div_scenic_spot_information.vue";
	import mixin from "@/mixins/page.js";
	export default {
		mixins:[mixin],
		components:{
			div_scenic_spot_information
		},
		data(){
			return{
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/scenic_spot_information/set?scenic_spot_information_id=' + obj["scenic_spot_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							...
							source_field: "scenic_spot_information_id",
							source_id: this.obj.scenic_spot_information_id,
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "scenic_spot_information",
					source_field: "scenic_spot_information_id",
					source_id: obj["scenic_spot_information_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.name_of_scenic_spot
					query.img = this.obj.cover_photo_
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/scenic_spot_information/set?scenic_spot_information_id=' + _this.obj["scenic_spot_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
