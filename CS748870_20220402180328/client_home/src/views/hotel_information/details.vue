<template>
	<div class="diy_details page_hotel_information" id="hotel_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_hotel_information'>
							<div_hotel_information :obj="obj"></div_hotel_information>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_hotel_information from "@/components/diy/div_hotel_information.vue";
	import mixin from "@/mixins/page.js";
	export default {
		mixins:[mixin],
		components:{
			div_hotel_information
		},
		data(){
			return{
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/hotel_information/set?hotel_information_id=' + obj["hotel_information_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "hotel_information",
					source_field: "hotel_information_id",
					source_id: obj["hotel_information_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.hotel_name
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/hotel_information/set?hotel_information_id=' + _this.obj["hotel_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
